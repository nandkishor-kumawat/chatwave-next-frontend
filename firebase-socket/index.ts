...
import { ref, set, onValue, remove, child, DatabaseReference, onDisconnect, update, onChildAdded } from "firebase/database";
...
class Fsocket {
    ...
    async emit(event: string, data: Record<string, any>) {
        ...
        try {
            let path = this._eventsRef
            // if (this.targetRoom) {
            //     path = child(this._roomsRef, `${this.targetRoom}/events`);
            //     this.targetRoom = undefined;
            // }
            const eventRef = child(path, event);
    ...
    async emitTo(event: string, data: Record<string, any>, room: string) {
        const eventData = {
            event,
            data,
    ...
    toRoom(room: string) {
        return {
            emit: async (event: string, data: any) => {
                this.targetRoom = room;
                await this.emitTo(event, data, room);
            }
        }
    }
